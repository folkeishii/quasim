...
pub fn apply_gate(state_vector: &mut DVector<Complex<f64>>, gate: &Gate) {
    ...
    for i in 0..n {
        if !is_block_base(i, targets) {
            continue;
        }

        if !controls_active(i, controls) {
            continue;
        }

        match gate.get_type() {
            GateType::X => apply_x(state_vector, i, targets),
            GateType::Y => apply_y(state_vector, i, targets),
            GateType::Z => apply_z(state_vector, i, targets),
            GateType::H => apply_h(state_vector, i, targets),
            GateType::S => apply_s(state_vector, i, targets),
            GateType::SWAP => apply_swap(state_vector, i, targets),
            GateType::U(theta, phi, lambda) => {
                apply_unitary2(state_vector, i, &get_u_matrix2(theta, phi, lambda), targets)
            }
        }
    }
}
