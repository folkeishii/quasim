...
/// Appends an oracle to the given circuit based on the provided ANF coefficients.
pub fn append_oracle<B: CircuitBehaviour>(
    ...
    for (i, coef) in anf_coefs.iter().enumerate() {
        ...
        if i == 0 {
            // Apply X gate for the constant term
            circuit = circuit.x(target);
            continue;
        }

        // Apply a multi-controlled X gate for the term corresponding to index i
        let control_qubits = controls_from_bit_mask(&input_qubits, i);
        circuit = circuit.cx(&control_qubits, target);
    }
